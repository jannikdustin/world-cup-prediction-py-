...
Braucht KEINEN Import aus deinem WM-Repo (elo.py etc.) -- clubelo.com liefert
die Wahrscheinlichkeiten direkt pro Tordifferenz. Trotzdem reicht dieselbe
Value-Logik wie im WM-Modell, deshalb wird `analyze_value` aus value_oracle.py
wiederverwendet.
...
3. Team-Namen zwischen Odds-API und ClubElo abgleichen (Fuzzy + Override-Tabelle,
   die beiden Quellen schreiben Klubnamen oft unterschiedlich, z.B.
   "Bayern Munich" (Odds API) vs. "Bayern" (ClubElo))
...
Voraussetzung: value_oracle.py muss im selben Ordner oder im Python-Path liegen.
...
# value_oracle.py liegt eine Ebene hoeher im Repo-Hauptordner. Muss VOR
# dem Import ergaenzt werden.
...
NAME_OVERRIDES = {
    ...
    # --- 2. Bundesliga (ClubElo nutzt kurze/ASCII-Schreibweisen, siehe
    # api.clubelo.com/Fixtures). Linke Seite = The-Odds-API-Name, rechte
    # Seite = exakte ClubElo-Schreibweise. Bei "kein ClubElo-Match" im
    # Dashboard hier gegen clubelo.com/Ranking abgleichen und ergaenzen. ---
...
def resolve_and_match(odds_home, odds_away, fixtures):
    ...
    clubelo_names = list({f["home"] for f in fixtures} | {f["away"] for f in fixtures})
...
def load_odds():
    if not os.path.exists(ODDS_PATH):
        print(f"FEHLER: {ODDS_PATH} nicht gefunden. Erst fetch_odds_clubs.py laufen lassen.",
              file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    for m in odds_matches:
        ...
        if not fixture:
            skipped.append(f"{m['home']} vs {m['away']} ({m['league']}) — kein ClubElo-Match")
            continue
    ...
    if skipped:
        print(f"{len(skipped)} Spiele uebersprungen (Team-Namen nicht gematcht):")
        for s in skipped:
            print(f"  - {s}")
